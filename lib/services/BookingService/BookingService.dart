...
class BookingService{
  ...
  Future<List<Doctor>> retrieveDoctorData() async{
    ...
    return snapshot.docs
        .map((docSnapshot) => Doctor.fromDocumentSnapshot(docSnapshot)).toList();
  }
  ...
  Future<void> updateBookingUserDataPhone(Booking book){
    return _firestore.collection('booking').where("id",isEqualTo: book.id).get().
    then((value){
      try{
          value.docs[int.parse(book.id)].data().update('phone', (value) => book.phone);
      }catch(e){
        print(e.toString());
      }
    });
  ...
  Future<void> updateBookingUserDataImagePath(Booking book){
    ...
    then((value){
      try{
        value.docs[int.parse(book.id)].data().update('path', (value) => book.phone);
      }catch(e){
        print(e.toString());
      }
    });
