  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<EmailBloc,EmailState>(
      listener: (context,state){
        if(state is Sended){
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: CustomSnackBarContent(errorText: "Email Sent Successfully")));
        }
      },
                      ...
                      children: <Widget>[
                        ...
                        Center(
                            child: OutlinedButton(
                              ...
                              onPressed: () {
                                for (index_id = 0; index_id <
                                    state.books.length; index_id++) {
                                  if (UserAuth.email ==
                                      state.books[index_id].email) {
                                    var bookingReg = BookingRegister(
                                      ...
                                      spec: state.spec[widget.id_spec].spec_name
                                          .toString(),
                                    ...
                                    var email = Email(
                                          ...
                                          message: "${bookingReg
                                              .toString()} Bệnh nhân: ${state.books[index_id].name} \n - Số điện thoại: ${state.books[index_id].phone} - Giới tính: ${state.books[index_id].gender} \nBác sĩ đảm nhiệm ${state
                                              .spec[widget.id_spec]
                                              .list_doctors[widget.id_doc]
                                              .toString()} thuộc khoa ${state
                                              .spec[widget.id_spec].spec_name
                                              .toString()}\n Vào lúc: ${timeController
                                              .value.text} ngày ${dateController.toLocal()
                                              .toString().split(' ')[0]}",
                                          toName: state.spec[widget.id_spec]
                                              .list_doctors[widget.id_doc]
                                              .toString(),
                                          toEmail: state.doctors[0].email,
                                        );

                                        setState(() {
                                          context.read<BookRegBloc>().add(
                                              AddBookingReg(bookingReg));
